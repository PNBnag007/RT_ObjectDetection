...
from config import COCO_IMG_MEAN


class CocoDetectorDataset(DataLoader.Dataset):
    image_size = 448
    ...
    def __init__(self, root_img_dir, dataset_file, train, S, preproc=True, return_image_id=False, encode_target=True):
        ...
        for line in lines:
            ...
            # num_len = len(split_line[0])-4
            # up_word = "0"*(12 - num_len) + split_line[0]
            # print("up_word = ",up_word)
            # if up_word in os.listdir('train2017'):
    ...
    def encoder(self, boxes, labels):
        ...
        boxes = [[x1,y1,x2,y2],[...]]
        labels = [...]
        return SxSx(5xB+C) (14x14x20 in our case we use 10 labels in COCO)
        '''
        grid_num = self.S
        target = torch.zeros((grid_num, grid_num, 20))
