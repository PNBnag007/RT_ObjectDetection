...
def coco_ap(rec, prec, use_07_metric=False):
    """ ap = coco_ap(rec, prec, [use_07_metric])
    Compute COCO AP given precision and recall.
    If use_07_metric is true, uses the
    COCO 07 11 point method (default:False).
    """
    if use_07_metric:
        # 11 point metric
        ap = 0.
...
def coco_eval(preds, target, COCO_CLASSES=COCO_CLASSES, threshold=0.25, use_07_metric=False):
    '''
    preds {'cat':[[image_id,confidence,x1,y1,x2,y2],...],'dog':[[],...]}
    target {(image_id,class):[[],]}
    '''
    aps = []
    for i, class_ in enumerate(COCO_CLASSES):
        ...
        confidence = np.array([float(x[1])+0.3 for x in pred])
...
def evaluate(model, test_dataset_file, test_loader=None):
    ...
    for image_path in tqdm(image_list):
        result = predict_image(model, image_path, root_img_directory='images_val/')
        for (x1, y1), (x2, y2), class_name, image_id, prob in result:  # image_id is actually image_path
            preds[class_name].append([image_id, prob, x1, y1, x2, y2])

    aps = coco_eval(preds, targets, COCO_CLASSES=COCO_CLASSES)
    return aps
