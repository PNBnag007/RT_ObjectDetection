...
def predict_image(model, image, root_img_directory=''):
    """
    ...
    """
    # print(image_name,"Root Image Directory",root_img_directory)
    result = []
    print("type of image Attribute",type(image))
    ...
    with torch.no_grad():
        ...
        for i, box in enumerate(boxes):
            ...
            prob = probs[i]
            prob = float(prob)
            result.append([(x1, y1), (x2, y2), VOC_CLASSES[cls_index], VOC_CLASSES[cls_index], prob])
            print("Result",result)
    return result
